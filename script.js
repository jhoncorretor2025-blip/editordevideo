...
async function processVideo() {
  ...
  try {
    ...
    setStatus('📊 Analisando silêncios...', 'active', 15);
    const segments = detectSpeechSegments(audioBuf, threshold, minSilMs, padMs);

    if (segments.length === 0) {
      setStatus('⚠️ Nenhum segmento encontrado. Reduza o threshold.', 'error', 0);
      resetBtn(); return;
    }

    drawSegmentMap(segments, audioBuf.duration);
    setStatus(`✂️ ${segments.length} segmentos. Codificando MP4...`, 'active', 25);
    ...
    if (typeof VideoEncoder !== 'undefined' && typeof Mp4Muxer !== 'undefined') {
      finalBlob = await encodeMP4(videoFile, segments, audioBuf, (p) => {
        setStatus(`🎬 Codificando frame ${p.cur} de ${p.total}...`, 'active', 25 + p.ratio * 65);
      });
    } else {
      // Fallback: MediaRecorder (WebM) em browsers sem WebCodecs
      setStatus('⚠️ WebCodecs não disponível. Usando WebM...', 'active', 25);
      finalBlob = await encodeWebM(videoFile, segments, (p) => {
        setStatus(`🎬 Capturando ${p.cur}/${p.total}...`, 'active', 25 + p.ratio * 65);
      });
    }
    ...
    setStatus(`✅ Pronto! ${pctRemoved}% removido. Arquivo: ${ext.toUpperCase()}`, 'success', 100);
